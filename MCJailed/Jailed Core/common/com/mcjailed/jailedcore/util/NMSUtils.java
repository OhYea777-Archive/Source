...
public class NMSUtils {
	...
	static {
		...
		if (class_PacketPlayOutMapChunkBulk == null
				&& class_Packet56MapChunkBulk == null) {
			// This should probably use a logger, but.. this is a pretty bad
			// issue.
			System.err
					.println("Could not bind to either PlayOutMapChunk packet version");
		}
		...
		if (class_PacketPlayOutWorldParticles == null
				&& class_Packet63WorldParticles == null) {
			// This should probably use a logger, but.. this is a pretty bad
			// issue.
			System.err
					.println("Could not bind to either PlayOutWorldParticles packet version");
		}
	}
	...
	protected static void sendPacket(Location source,
			Collection<Player> players, Object packet) throws Exception {
		players = ((players != null && players.size() > 0) ? players : source
				.getWorld().getPlayers());

		for (Player p1 : players) {
			if (p1.getLocation().distanceSquared(source) <= Bukkit.getServer()
					.getViewDistance() * Bukkit.getServer().getViewDistance()) {
	...
	protected static void sendPacket(Player player, Object packet)
		...
		Field connectionField = playerHandle.getClass().getField(
				"playerConnection");
		Object connection = connectionField.get(playerHandle);
		Method sendPacketMethod = connection.getClass().getMethod("sendPacket",
				class_Packet);
